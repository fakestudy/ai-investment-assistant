...
项目采用 monorepo 组织，整体由 Web 前端、Go 后端服务、Python Agent Service、PostgreSQL 和 Docker Compose 组成。
...
- 前端只通过 HTTP 调用 Go BFF，不直接访问内部服务。
...
| 层级 | 技术 |
| --- | --- |
| Web | Next.js App Router、React、TypeScript、TanStack Query、Tailwind CSS、Recharts |
| BFF/业务服务 | Go 1.26、grpc-go、pgx、chi、JWT |
| AI Agent | Python 3.12、LangGraph、LangChain、Pydantic |
...
- **Web App**：负责登录页、工作台布局、股票详情、研究卡片、AI 对话、自选股和通知展示。
- **BFF**：提供 Web 所需 HTTP API，处理认证、DTO 聚合和内部 gRPC 编排。
...
- **Agent Service**：基于 LangGraph/LangChain 生成事件分析、上下文问答和自选股摘要。
